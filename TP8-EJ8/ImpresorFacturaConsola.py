...
class ImpresorFacturaConsola:
    def imprimir(self, factura: Factura):
        ...
        for linea in factura.lineas:
            neto_linea = linea.calcular_neto_con_descuento(factura.descuentos)
            total_linea = linea.calcular_total_linea(factura.descuentos)
            ...
            if linea.descuentos:
                promos = ", ".join(d.descripcion for d in linea.descuentos)
                print(f"{'':<15} |      -> Promo: {promos}")

        print("---------------------------------------------------------------")
        print(f"Total Neto Gravado:                        ${factura.calcular_total_neto():<10.2f}")

        if factura.descuentos:
            print("Descuentos Globales Aplicados:")
            for descuento in factura.descuentos:
                print(f"  -> {descuento.descripcion}")
